...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('title') Activity Logs @endslot
    @endcomponent
            ...
            <div class="card card-animate">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4">
                                {{ number_format($stats['tasks_events']) }}
                            </h4>
                        </div>
                        <div class="avatar-sm flex-shrink-0">
    ...
    <div class="card">
        <div class="card-header border-0">
            <div class="row g-3 align-items-center">
                ...
                <div class="col-md-2">
                    <input type="date" class="form-control" name="date_from" id="date-from" value="{{ request('date_from') }}" onchange="filterActivities()">
                </div>
                <div class="col-md-2">
                    <input type="date" class="form-control" name="date_to" id="date-to" value="{{ request('date_to') }}" onchange="filterActivities()">
                </div>
                <div class="col-md-3">
                            ...
                            <tr>
                                <td>
                                    <small class="text-muted">
                                        ...
                                        <br>
                                        <span class="badge bg-light text-dark">{{ $activity->created_at->diffForHumans() }}</span>
                                    </small>
                                </td>
                                <td>
                                    @if($activity->causer)
                                ...
                                <td>
                                    <span class="badge bg-{{ $activity->log_name === 'tasks' ? 'primary' : ($activity->log_name === 'ai' ? 'info' : 'secondary') }}">
                                ...
                                <td>
                                    <a href="{{ route('management.activity-logs.show', $activity) }}" class="btn btn-sm btn-ghost-primary">
...
function filterActivities() {
    ...
    window.location.href = '{{ route('management.activity-logs.index') }}?' + params.toString();
}
</script>
@endsection
